...
public class MountainData {
    ...
    static ArrayList<Mountain> getListData() {
        ArrayList<Mountain> list = new ArrayList<>();
        for (int position = 0; position < GunungNames.length; position++) {
            Mountain mountain = new Mountain();
            mountain.setName(GunungNames[position]);
            mountain.setDetails(GunungDetails[position]);
            mountain.setPhoto(String.valueOf(GunungImages[position]));
            list.add(mountain);
        }
        return list;
    }
